...
def make_key(chunk_hash: int, model: str = "model", kv_rank: int = 0) -> ObjectKey:
    ...
    return ObjectKey(chunk_hash=chunk_hash, model_name=model, kv_rank=kv_rank)
...
class TestLRUEvictionPolicyBasic:
    ...
    def test_delete_keys_decreases_tracked_count(self):
        ...
        keys = [make_key(1), make_key(2), make_key(3)]
        policy.on_keys_created(keys)
        policy.on_keys_deleted([make_key(2)])
    ...
    def test_delete_nonexistent_key_is_safe(self):
        ...
        policy = LRUEvictionPolicy()
        policy.on_keys_deleted([make_key(999)])
        assert policy.get_num_tracked_keys() == 0
...
class TestLRUEvictionPolicyOrdering:
    ...
    def test_eviction_order_is_lru(self):
        ...
        assert candidates[0].chunk_hash == 1
        assert candidates[1].chunk_hash == 2
        assert candidates[2].chunk_hash == 3
    ...
    def test_touch_moves_key_to_most_recent(self):
        ...
        candidates = policy.get_eviction_candidates(3)
        assert candidates[0].chunk_hash == 2
        assert candidates[1].chunk_hash == 3
        assert candidates[2].chunk_hash == 1
...
class TestLRUEvictionPolicyRatio:
    ...
    def test_max_batch_size_limits_eviction(self):
        """Eviction should be limited by max_eviction_batch_size."""
        policy = LRUEvictionPolicy(max_eviction_batch_size=5)
        policy.on_keys_created([make_key(i) for i in range(100)])
        actions = policy.get_eviction_actions(1.0)
        assert len(actions) == 1
        assert len(actions[0].keys) == 5
